...
插件会按设定间隔（默认 15 分钟）检查所有已绑定账号的明日方舟理智，**理智回满时私聊提醒**。

- 防重复：提醒过一次后会标记，等你把理智用掉后才会在下次回满时再次提醒
- 个人开关：对机器人说"关闭理智提醒"可以只关掉你自己的提醒，不影响别人
- AI 查询：直接问"我现在多少理智""理智什么时候回满"，机器人会实时查询并告诉你预计回满时间
- WebUI 管理页的账号列表会显示最近一次检查到的理智缓存和提醒状态
- 通过 WebUI 网页绑定的账号没有会话记录，无法接收私聊提醒，但理智缓存仍会显示在管理页

> 终末地暂不支持：森空岛官方接口目前不返回终末地的体力数据（实测返回"未找到已绑定的游戏角色信息"），如后续官方开放会跟进支持。
