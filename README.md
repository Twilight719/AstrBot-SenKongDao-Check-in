...
管理页还支持：查看所有已绑定账号及其游戏/最近签到日期、单个或全部手动签到、解绑账号、修改自动签到设置、按账号开关理智提醒，以及**为每个账号选择提醒方式（私聊 / 指定群聊）**——群聊提醒会自动 @ 你，可选群来自机器人收到过消息的群（先把机器人拉进群并在群里发条消息即可被识别）。接口不会回传 token，网页上也看不到已保存的 token。
...
| `ap_remind_enabled` | 开 | 理智回满提醒总开关（明日方舟 + 终末地） |
| `ap_check_interval` | 15 | 理智检查间隔（分钟，5-120，建议 10-30，过短可能触发风控） |

开启后，到点会自动签到并把结果按每个账号设置的提醒方式（私聊或群聊）投递给对应用户。

## 提醒方式：私聊 / 群聊

每个账号可以单独选择提醒的投递方式（自动签到结果、理智回满提醒都按此投递），在插件的 WebUI 管理页「已绑定账号」列表的**提醒方式**列设置：

- **私聊**（默认）：提醒私聊发给你。前提是机器人和你有过私聊——私聊机器人发任意一条消息（比如 `/skd`）即可自动开通
- **群聊**：提醒发到指定群里并 @ 你。设置步骤：
  1. 把机器人拉进你想接收提醒的群
  2. 在那个群里**任意发一条消息**（插件通过群消息识别机器人所在的群）
  3. 打开 WebUI 管理页，在该账号的「提醒方式」列选择「群聊」，右侧下拉选择群号，自动保存生效

如果目标群变得不可达（例如机器人被移出群），提醒会自动回退为私聊，不会丢失。
